...
fn test_kitty_unicode_placeholder_basic() {
    ...
    // Diacritics are 1-based: U+0305=1, U+030D=2, U+030E=3 (0=not specified)
    // Cell 1: U+10EEEE + U+0305 (row=1) + U+0305 (col=1)
    // Cell 2: U+10EEEE + U+0305 (row=1) + U+030D (col=2)
    let placeholder =
...
fn test_kitty_unicode_placeholder_multirow_texcoords() {
    ...
    // Diacritics are 1-based: U+0305=1, U+030D=2, U+030E=3, U+0310=4
    let row1 = format!(
        "\x1b[38;2;0;0;1m\
    ...
    // Row 2 (diac row=2): 3 placeholders
    let row2 = format!(
...
fn test_kitty_unicode_placeholder_bottom_of_screen() {
    ...
    // Row 1 of image at screen row 2
    // Diacritics are 1-based: U+0305=1, U+030D=2, U+030E=3, U+0310=4
    term.print("\r\n\r\n"); // move to row 2
    let row1 = format!(
        "\x1b[38;2;0;0;1m\
    ...
    // The last visible row should have images since it was written last
    // (after scrolling pushed earlier rows up).
    let lines = term.screen().visible_lines();
    ...
    assert!(
        last_line
            ...
            .map(|c| c.attrs().images().is_some())
            .unwrap_or(false),
        "last visible row should have an image after scrolling"
    );
}
...
fn test_kitty_position_based_delete() {
    ...
    assert!(
        has_image(&term, 0, 0),
        "image should be at (0,0) after placement"
    );
    ...
    assert!(
        has_image(&term, 3, 2),
        "image should be at (3,2) after placement"
    );

    // Delete at (3,2) using d=p with x=4,y=3 (1-based per kitty protocol)
    term.print("\x1b_Ga=d,d=p,x=4,y=3,q=2\x1b\\");
    assert!(
        !has_image(&term, 3, 2),
    ...
    // Delete all placements intersecting column 5 (1-based: x=6)
    term.print("\x1b_Ga=d,d=x,x=6,q=2\x1b\\");
    assert!(
        !has_image(&term, 5, 0),
    ...
    // Delete all placements intersecting row 3 (1-based: y=4)
    term.print("\x1b_Ga=d,d=y,y=4,q=2\x1b\\");
    assert!(
        !has_image(&term, 0, 3),
    ...
    // Delete only z=10 at (0,0) -- image with z=20 should remain (1-based: x=1,y=1)
    term.print("\x1b_Ga=d,d=q,x=1,y=1,z=10,q=2\x1b\\");
    ...
    // Now delete z=20 at (0,0) (1-based: x=1,y=1)
    term.print("\x1b_Ga=d,d=q,x=1,y=1,z=20,q=2\x1b\\");
    assert!(
        !has_image(&term, 0, 0),
